...
| 场景 | Skill | 作用 |
| --- | --- | --- |
| CLI | **hap-cli** | 入口：装好并登录 `hap` 命令行，之后用 `hap guide` 拿到当前版本的命令地图与取值规范（待办、日程、动态、聊天、应用数据增删改） |
...
带你把 `hap` 装好、登录、选好组织与应用，然后交给命令行自己的 `hap guide`——命令地图和写记录的取值规范都由它现场生成，永远跟这台机器上装的版本一致，不会因为技能文档过期而对不上。当你的需求需要整体建/改应用或复杂查数时，它会指明该切到下面哪个专门技能。
...
适合「把想要的数据查出来」：讲清统一的筛选条件写法与各处可用的比较方式、透视的维度与聚合参数，给出可直接套用的模板，帮你写对复杂的 `--filter-json` / 透视查询。
