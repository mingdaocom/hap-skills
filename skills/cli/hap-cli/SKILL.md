...
当需求不是「单点操作」，而是**围绕一个应用做整体建设或结构性修改**时，不要手敲一长串命令——切到专门 skill。
...
- **它做什么**：先与用户确认业务方案，再物理搭建应用并填充示例数据；MCP 盖不到的少量增强配置在收尾阶段自动用 `hap` 命令补齐。
- **怎么用**：直接调用 `hap-mcp-app-builder` skill。它需要连接明道云 MCP 服务。
...
> **怎么选**：
> - **从零搭整个应用** → `hap-mcp-app-builder`（需明道云 MCP 服务）
